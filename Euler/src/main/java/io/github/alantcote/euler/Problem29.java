...
/**
 * A solution for Project Euler Problem 29. The problem statement is
...
public class Problem29 {
	 ...
	 * Derivation:
	 * 
	 * This is all about factors. We know that if x = y ** z, then x can be factored
	 * into z copies of y. If we consider the prime factors of y, then each such
	 * factor of y appears z times as often in the prime factorization of x.
	 * 
	 * Another approach looks at w ** x = y ** z. Observe that w and y must have the
	 * same prime factors, ignoring repetition. If we then pay attention to the
	 * repetitions of them, the factorization of y must have x/z times the number of
	 * instances of any given prime factor as w does. Note that if w and y don't
	 * have the same prime factors in them, then no x and z can make the equation
	 * valid.
	 * 
	 * I'm baffled, so I'm inclined to go brute force. The hard thing, though, is
	 * that we can't actually calculate these beasts with longs; we'd have to use
	 * BigInteger. Bummer - that'd be slow. We can't accumulate in a Set, either;
	 * there'll be something like 100 ** 100 of them.
	 * 
	...
	public int calc() {
		...
		for (int i = 0; i < facts.length; ++i) {
			System.out.println("{" + facts[i].getN() + "} " + Arrays.toString(facts[i].getUniqueFactors()) + " "
					+ Arrays.toString(facts[i].getFactorCounts()) + " " + Arrays.toString(facts[i].getPrimeFactors()));
		}
